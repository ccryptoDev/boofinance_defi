...
@Component({
  selector: 'app-vault-composition',
  templateUrl: './vault-composition.component.html',
  styleUrls: ['./vault-composition.component.scss'],
  animations: [
...
export class VaultCompositionComponent implements OnInit, OnDestroy {
  // Subscriptions
  tokens$: Subscription;
  vaultComposition$: Subscription;
  // Subscription variables
  tokens: Map<string, Token>;
  // Menu variables
  breakPoints: NbMediaBreakpoint[];
  chartType = ChartType.PieChart;
  vaultComposition: CauldronComposition | null;
  ...
  ngOnInit(): void {
    this.chartResize();
    this.tokens$ = this.tokensService.getSingleTokens().subscribe((tokens) => {
      this.tokens = tokens;
    });
  ...
  getSingleTokenComposition() {
    this.vaultComposition$?.unsubscribe();
    this.vaultComposition$ = this.cauldronService.getSingleTokenComposition(false, this.tokens).subscribe(vaultComposition => {
      if (vaultComposition) {
        this.reloadChartData(vaultComposition.tokensDetailsMap);
        this.vaultComposition = vaultComposition;
      }
    });
  }
