...
import {VaultTokenSelectionMenu, VaultContributionMenu} from "../../@enums/vault-enum-menu";
...
import {VaultCompositionComponent} from "../@modals/vault-composition/vault-composition.component";
...
export class VaultComponent implements OnInit {
  ...
  VaultTokenSelectionMenuEnum = VaultTokenSelectionMenu;
  VaultContributionMenuEnum = VaultContributionMenu;
  ...
  vaultTokenSelectionMenu: VaultTokenSelectionMenu = VaultTokenSelectionMenu.UserTokens;
  cauldronHarvestSelectionMenu: CauldronHarvestSelectionMenu = CauldronHarvestSelectionMenu.All;
  vaultContributionMenu: VaultContributionMenu = VaultContributionMenu.Contribute;
  ...
  ngOnInit(): void {
    ...
    this.tokens$ = this.tokensService.getSingleTokens().subscribe(
      tokenList => {
    ...
    this.user$ = this.authenticationService.getConnectedUser().subscribe(user => {
      ...
      this.vaultContributionMenu = VaultContributionMenu.Contribute;
    ...
    this.getSingleTokenComposition();
  }
  ...
  **/
    getSingleTokenComposition() {
    this.cauldronComposition$?.unsubscribe();
    this.cauldronComposition$ = this.cauldronService.getSingleTokenComposition(true, this.tokens).pipe(
  ...
  **/
  getUserTokens() {
    ...
    this.userTokens$ = this.onUserConnectedAndExecutesTransaction().pipe(
      concatMap(([user, transaction]) => {
        return combineLatest([
          this.tokensService.getSingleTokens(),
      ...
      if (userTokens.size == 0 && !this.userUpdatedCauldronSelectionMenu) {
        this.vaultTokenSelectionMenu = VaultTokenSelectionMenu.LiquidityPoolTokens;
      }
      this.loadingUserTokens = false;
  ...
  **/
  openVaultCompositionDialog(): void {
    this.nbDialogService.open(VaultCompositionComponent, {
  ...
  **/
  onHelpToggled(): void {
    ...
    newConfiguration.helpToggles.howToUseVault = !newConfiguration.helpToggles.howToUseVault;
    this.userConfigurationService.setUserConfiguration(newConfiguration);
  }

  /**
    Updates the active "Select a token" tab
    @param vaultTokenSelectionMenu The Enum index which the Tab we want to display
  **/
  setTokenSelectionTab(vaultTokenSelectionMenu: VaultTokenSelectionMenu): void {
    this.userUpdatedCauldronSelectionMenu = true;
    this.vaultTokenSelectionMenu = vaultTokenSelectionMenu;
  }
